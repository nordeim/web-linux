The user wants me to act as a file search specialist and have a deep understanding of the project by reviewing the four documentation files. They want me to understand the WHAT, WHY, and HOW of the project and its codebase design and architecture.
Looking at the request, the user has already provided the content of CLAUDE.md, AGENTS.md, GEMINI.md, and README.md. They want me to demonstrate deep understanding.
The key question is: what is the user actually asking for? They've provided the documents and asked me to "meticulously review" them to have a deep understanding. This seems like an exploratory request — they want me to acknowledge my understanding and potentially confirm I can act on it.
Given the context (file search specialist role), I think the user wants me to:
1. Confirm I've reviewed the documents
2. Summarize my deep understanding of WHAT, WHY, HOW
3. Be ready to perform file search tasks
But the request is somewhat ambiguous. Let me re-read: "please meticulously review @CLAUDE.md , @AGENTS.md , @GEMINI.md and @README.md to have a deep understanding of the WHAT, WHY and HOW of the project and its codebase design and architecture. You will act as a file search specialist."
So they want me to:
- Review the four docs
- Have deep understanding
- Act as file search specialist
The user has not yet asked a specific search question. They're setting up context. I should acknowledge my understanding concisely (per the system prompt: be concise, fewer than 4 lines unless asked for detail).
Actually, looking more carefully — the documents have already been read (the tool calls show the content). The user is essentially saying "here are the docs, now act as file search specialist."
I should provide a brief acknowledgment of my understanding and indicate readiness to perform file searches. Keep it concise.
Let me summarize the key understanding:
WHAT: UbuntuOS Web — a browser-based replica of Ubuntu Linux desktop. SPA built with React 19.2, TypeScript 5.9, Vite 7.2, Tailwind 3.4. Has custom window manager, virtual file system (VFS) with localStorage persistence, 55 functional apps across 7 categories.
WHY: Dual purpose — developer showcase of advanced React patterns (Context+useReducer, windowing, z-index stacking) AND a portable web-accessible toolset.
HOW:
- State: useOSStore.tsx (React Context + useReducer) — global OS state (windows, z-index, focus, desktop icons)
- VFS: useFileSystem.ts — ID-based, localStorage (ubuntuos_filesystem_v2)
- Windowing: WindowFrame.tsx — custom chrome, drag/resize/focus
- Code splitting: React.lazy() + Suspense in AppRouter.tsx (~1MB → ~360KB)
- Security utilities in src/utils/: safeEval (shunting-yard), sanitizeHtml (DOMPurify), storageValidation (zod), safeJsonParse
- Strict TS: noUnusedLocals + noUnusedParameters enforced
- Build commands (from app/): npm run dev/build/lint/test/preview
Key entry points:
- src/hooks/useOSStore.tsx
- src/hooks/useFileSystem.ts
- src/apps/AppRouter.tsx
- src/utils/safeEval.ts
- src/utils/sanitizeHtml.ts
- src/utils/safeJsonParse.ts
- src/utils/storageValidation.ts
- src/components/GlobalErrorBoundary.tsx
- src/components/DynamicIcon.tsx (only file allowed wildcard lucide import)
- src/components/NotImplemented.tsx
Given the instructions to be concise (<4 lines unless detail requested), and the user hasn't asked a specific search question, I should briefly acknowledge readiness and summarize the core understanding.
...
The user said "meticulously review" — this implies they want detail. But they also said "act as a file search specialist" which suggests they want me to be ready for searches.
I think the best response is a concise structured summary that demonstrates I understand the project deeply, then indicate readiness. Given the user said "meticulously" I'll provide a slightly longer summary covering WHAT/WHY/HOW and the key technical patterns, but still focused.
Let me draft a structured response.
Project Understanding: UbuntuOS Web
WHAT
A browser-based replica of the Ubuntu Linux desktop environment. SPA built with React 19.2, TypeScript 5.9 (strict), Vite 7.2, Tailwind 3.4, Shadcn/Radix UI, Lucide React, DOMPurify, Zod, and Vitest. Features a custom window manager, a persistent Virtual File System (VFS) (localStorage key ubuntuos_filesystem_v2), and 55 functional apps across 7 categories (System, Productivity, Internet, Media, Games, DevTools, Creative). Code-split via React.lazy()+Suspense (initial bundle ~360 KB).
WHY
Dual purpose: (1) Developer showcase of advanced React architecture without external state libraries (Context+useReducer, windowing, z-index stacking); (2) Portable web-accessible toolset of common utilities. Architecture was driven by the need to avoid eager-loading 54+ apps and to keep state manageable.
HOW
- OS Store: src/hooks/useOSStore.tsx — React Context + useReducer (~350 lines). Manages windows, z-index (nextZIndex, capped at 2147483647), focus, minimize/maximize/restore, notifications, context menu, alt-tab, desktop icons. Side effects (localStorage) extracted to useEffect.
- VFS: src/hooks/useFileSystem.ts — ID-based (not path-based), path normalization, FILE_ASSOCIATIONS map, trash handling, synced to localStorage.
- Windowing: src/components/WindowFrame.tsx — standardized chrome; drag/resize/focus at the frame level.
- Routing: src/apps/AppRouter.tsx — lazy-loaded apps + NotImplemented fallback.
- Security utilities in src/utils/: safeEval (shunting-yard parser, replaces eval/new Function), sanitizeHtml/sanitizeMarkdownHtml (DOMPurify wrappers), storageValidation (zod for OS state), safeJsonParse(raw, schema, fallback) (ad-hoc app validation).
- Build hygiene: tsconfig.app.json enforces noUnusedLocals + noUnusedParameters (43 TS6133 errors fixed 2026-06-02). Named lucide imports only — src/components/DynamicIcon.tsx is the sole authorized wildcard importer.
...
Conventions / Anti-Patterns (must enforce)
- Forbidden: eval(), new Function(), raw JSON.parse(localStorage), raw dangerouslySetInnerHTML, wildcard import * as Icons from 'lucide-react' (except DynamicIcon.tsx), dead/unused imports.
- Mandatory: safeEval for math, sanitizeHtml for HTML, safeJsonParse/storageValidation for localStorage, MAX_EXEC_ITERATIONS=1000 for user regex, cap factorial at 170, aria-label on icon buttons.
- Commands (from app/): npm run dev | build | lint | test | preview; tsc -b for type-check.
